...
export interface ColorTermsSyncRequest {
  dryRun?: boolean
  realtime?: boolean
  prune?: boolean
...
export interface ColorTermsSyncStats {
  dryRun: boolean
  prune: boolean
  windows: number
...
export interface ColorTermReport {
  code: string
  name: string
  rgb: string | null
...
export interface ColorTermsProgress {
...
export type ColorTermsProgressHandler = (event: ColorTermsProgress) => void | Promise<void>
...
async function fetchVestiProducts(
  ...
  for (let windowStart = start; windowStart <= end; windowStart = addDays(windowStart, windowDays)) {
    ...
    for (let page = 1; page <= 200; page += 1) {
      const endpoint =
        `${DEFAULT_ENDPOINTS.vesti.products}?start_date=${formatDate(windowStart)}%2000:00:00` +
        `&end_date=${formatDate(windowEnd)}%2023:59:59&perpage=${req.perPage}&page=${page}`
...
async function pruneExtraColorTerms(
  ...
  dryRun: boolean,
  stats: ColorTermsSyncStats,
  onProgress?: ColorTermsProgressHandler,
) {
...
export async function runColorTermsSync(body: ColorTermsSyncRequest, onProgress?: ColorTermsProgressHandler) {
